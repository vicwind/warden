...
* Install Bundler <code> gem install bundler</code>, and install all the require gems by doing <code> bundle install</code>
* Setup WARDEN_CONFIG_DIR & WARDEN_HOME environment variables in the <code><warden dir>/.rvmrc</code> file to point the project root dir and it’s config dir
<pre><code>Example:
...
* Test run your setup:
** try to run Cucumber
<pre><code>cucumber project/test_project/features/buying_guide.feature
</code></pre>
...
h2. Create a New Test Project (Amazon Book Search)
* Create a scaffolding of the new project
<pre><code>bash <warden root>/create_project.sh amazon_search_test</code></pre>
* It will create a project skeletons like the following in the created project directory
<pre>drwxr-xr-x  2 victor  staff   68 Feb 26 15:55 etc # other project related assets, eg. locale.yml
...
* Edit the <code> <warden root>/config/app_env.yaml</code> file. add an entry under development section
<pre><code>#for this example, we added amazon_serach_test and it's URL. all the features in the project will start with that url
  development: 
    'amazon_search_test': http://www.amazon.com/
</code></pre>
* Edit the <code> <test project dir>/features/project_init.rb</code> file, and do the following change 
<pre><code>require "#{ENV['WARDEN_HOME']}/config/env"
...
* Open your favorite editor and create your every first Cucumber feature file :) create the new file in the "features" directory <code><project root>/features/test_search.feature</code>
<pre><code>Feature:
...
* Run the feature: <pre><code>cucumber ./test_search.feature</code></pre>
* Cucumber will take you that you have undefined steps. create an new step files <code><project root>/features/test_search_steps.rb</code>
* Copy the Cucumber suggested steps definition to the file
<pre><code>When /^the user type "([^"]*)" in the search bar and click "([^"]*)"$/ do |arg1, arg2|
...
<pre><code>wb-preset:features victor$ cucumber ./test_search.feature
Feature: 
  Test Amazon's product search feature
